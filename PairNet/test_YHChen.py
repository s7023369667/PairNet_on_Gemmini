...
def test(f, model, true_label, test_base, test_index, model_name):
    ...
    if len(guess) != len(true_label):
        ...
        print('{}True Label: {} ; Predict: {} -> '.format(' ' * 6, true_label, guess), end='')
...
if __name__ == '__main__':
    ...
    model_name = './model/pairnet_model16_12_20220503.h5'
    ...
    for test_index in range(0, len(test_base)):
        ...
        for label in path_list:  # label : '1-2', '1-3', ...
            sub_dir = os.path.join(test_base_dir, label)  # 'test\test3\1-2', 'test\test3\1-3', ...
            print(sub_dir)
            for file in os.listdir(sub_dir):  # 'SensorData_2017_11_21_161824.txt', ....

                if '.txt' not in file:  # 副檔名不是txt -> 不進行處理(非預期裝手勢的檔案)
                    continue
                file_path = os.path.join(test_base_dir, label)  # 'test\test3\1-2'
                ...
                ##FIXME
                test(file_path, model, true_label, test_base, test_index, model_name)
            #
