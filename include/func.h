...
static void conv1d_matmul(size_t batch_size, size_t input_width, size_t in_channels,
                   const elem_t in_feature[batch_size][1][input_width][in_channels],
                   size_t kernel_size, size_t out_channels,size_t stride_size,
                   const elem_t conv1d_weight[1][kernel_size][in_channels][out_channels],
                   size_t padding_front, size_t padding_back, size_t output_width,
                   const acc_t total_bias[batch_size][1][output_width][out_channels],double downScalar,
                   elem_t Z3, elem_t Z4,elem_t out_feature[batch_size][1][output_width][out_channels]){
    /**reshape kernel**/
    elem_t reshape_kernel[kernel_size * in_channels][out_channels];
    for (int i = 0; i < kernel_size; ++i) {
        for (int j = 0;j < in_channels;++j) {
            for (int k = 0;k < out_channels;++k) {
                reshape_kernel[(i* in_channels)+j][k] = conv1d_weight[0][i][j][k];
            }
        }
    }
    bool padding_front_flag = (padding_front !=0) ? true : false;
...
static void conv1d_TF_folding(size_t batch_size, size_t input_width, size_t in_channels,
                       ...
                       double S4, elem_t Z4,elem_t out_feature[batch_size][1][output_width][out_channels]) {
...
//        }
        for (int i = 0; i < output_width; ++i) {
            ...
            for (int j = 0; j < out_channels; ++j) {
                ...
                out_feature[batch_idx][0][i][j] = QRelu_Clip(round_near_even((((S1 * S2) / S4 ) * (tmp_res + total_bias[i][j]))),Z3, Z4, true);
//                out_feature[batch_idx][0][i][j] = round_near_even((((S1 * S2) / S4 ) * (tmp_res + total_bias[i][j])));
 ...
 static void Qglobal_avg_pooling(int batch_size, int input_width, int in_channels, elem_t input_feature[batch_size][1][input_width][in_channels],
